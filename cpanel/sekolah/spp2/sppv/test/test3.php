...
var KODSEKOLAH = "<?php echo $_SESSION['SESS_KODSEKOLAH']; ?>";
    for (i = 0; i < clientkp.length; i++) { 
      html += (i+1) + " : [ "+clientkp[i]+" ] " +showUser(clientkp[i])+ " <button  onclick='del2("+i+");'>X</button><br>";
    }
...
function showfor()
{
              // create an array
  var KODSEKOLAH = "<?php echo $_SESSION['SESS_KODSEKOLAH']; ?>";
  var html="Jumlah Murid dipilih : "+clientkp.length + "<button onclick='showfor2();' >Papar</button>";

    for (i = 0; i < clientkp.length; i++) { 
      html += '<input type="hidden" name="s_nokp[]" value="'+clientkp[i]+'">';
    }
...
$connect = new PDO("mysql:host=localhost;dbname=ftkluang_taskforce", "ftkluang", "r#587:fZnpJWX9")or die ("Pengkalan Data Tidak Dapat Dipilih.");
session_start();
...
<table width="40%" height="125" border="1" align="center">
  ...
  <tr>
    ...
    <tr>
    <td height="23" align="left" bgcolor="#33CCCC">  <div class="row">
