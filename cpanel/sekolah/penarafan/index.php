...
function skor($PPD,$bulan,$jenis){
      ...
      $kuri = $PPD->prepare("SELECT ((sum(d.value)/sum(p.radio))*100) jumlah FROM penarafan_bulan b inner join`penarafan_data` d on b.ID=d.data_id inner join penarafan_perkara p on p.ID=d.perkara_id where p.data='radio' and p.kategori=? and MONTH(b.tarikh)=? and b.kodsekolah=?");
